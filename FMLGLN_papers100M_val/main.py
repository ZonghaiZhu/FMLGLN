...
def main():
    ...
    test_idx = torch.cat((train_idx, valid_idx))
    ...
    feats_tst = feats_tst[len(train_idx):]
    if args.single_class:
        targets_tst = data.y[valid_idx].view(-1)
    else:
        targets_tst = np.vstack([data.y[i] for i in valid_idx])
